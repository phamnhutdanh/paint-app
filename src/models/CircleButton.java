...
public class CircleButton extends JButton implements ActionListener, MouseListener, MouseMotionListener {
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (canvasPanel.isDragged()) {
            if (x1 < x2 && y1 < y2) {
                canvasPanel.pushStackForShapes(new Shape(x1, y1, x2 - x1, y2 - y1,primary,stroke, SHAPES.CIRCLE,secondary, transparent));
                //	graphics2D.draw(new Ellipse2D.Double(x1, y1, x2 - x1, y2 - y1));
            }
            else if (x2 < x1 && y1 < y2) {
                canvasPanel.pushStackForShapes(new Shape(x2, y1, x1 - x2, y2 - y1,primary,stroke, SHAPES.CIRCLE,secondary, transparent));
                //graphics2D.draw(new Ellipse2D.Double(x2, y1, x1 - x2, y2 - y1));
            }
            else if (x1 < x2 && y2 < y1) {
                canvasPanel.pushStackForShapes(new Shape(x1, y2, x2 - x1, y1 - y2,primary,stroke, SHAPES.CIRCLE,secondary, transparent));
                //graphics2D.draw(new Ellipse2D.Double(x1, y2, x2 - x1, y1 - y2));
            }
            else if (x2 < x1 && y2 < y1) {
                canvasPanel.pushStackForShapes(new Shape(x2, y2, x1 - x2, y1 - y2,primary,stroke, SHAPES.CIRCLE,secondary, transparent));
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        boolean transparent = canvasPanel.isTransparent();
        if (x1 < x2 && y1 < y2) {
           canvasPanel.pushStackForPreview(new Shape(x1, y1, x2 - x1, y2 - y1,primary,stroke, SHAPES.CIRCLE,secondary,transparent));
            //graphics2D.draw(new Ellipse2D.Double(x1, y1, x2 - x1, y2 - y1));
        }
        else if (x2 < x1 && y1 < y2) {
            canvasPanel.pushStackForPreview(new Shape(x2, y1, x1 - x2, y2 - y1,primary,stroke, SHAPES.CIRCLE,secondary,transparent));
            //graphics2D.draw(new Ellipse2D.Double(x2, y1, x1 - x2, y2 - y1));
        }
        else if (x1 < x2 && y2 < y1) {
            canvasPanel.pushStackForPreview(new Shape(x1, y2, x2 - x1, y1 - y2,primary,stroke, SHAPES.CIRCLE,secondary,transparent));
            //graphics2D.draw(new Ellipse2D.Double(x1, y2, x2 - x1, y1 - y2));
        }
        else if (x2 < x1 && y2 < y1) {
